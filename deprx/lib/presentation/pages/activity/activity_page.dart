...
class _ActivityPage extends State<ActivityPage> {
  ...
  @override
  void initState() {
    GAUtil.logScreen(screenName: GAScreenList.activityMain);
    _controller.getProgramInfo();
    _controller.initMonthBA(_controller.selectedData.value);
    super.initState();
  ...
  Widget _dataList() {
    ...
    if (existing) {
      List<BAModel> data =
          _controller.getSelectedDayList(_controller.selectedData.value);
      if (data.isNotEmpty) {
        return Column(
          mainAxisAlignment: MainAxisAlignment.start,
          crossAxisAlignment: CrossAxisAlignment.center,
            ...
            Padding(
              ...
              child: Column(
                ...
                children: List.generate(data.length, (idx) {
                  BAModel item = data[idx];
                  return Padding(
                    padding: EdgeInsets.only(
                        bottom: idx != data.length - 1 ? 12 : 0),
                    child: BAItemContainer(
                      data: item,
                      function: () {
                        GAUtil.trackEvent(
                            name: GAEventList.ACITIVTY_ITEM_CLICK,
                            params: {GAParameter.ACTIVITY_NAME: item.contents});
                        DateTime now = DateTime.now();
                        if (item.done == false &&
                            isSameDay(now, _controller.selectedData.value)) {
  ...
  Widget _dateLabel() {
    String monthDay =
        TimeUtil.convertDateTo(_controller.selectedData.value, 'MM/dd');
    String weekOfDay = TimeUtil.getDayOfWeek(_controller.selectedData.value);
    return SizedBox(
      width: _screenUtil.width,
      child: Padding(
      ...
      child: Container(
        ...
        child: Column(
          ...
          children: [
            ...
            Text(
              isAfter
                  ? 'common_yetEmptyActivity'.tr
                  : 'common_emptyActivity'.tr,
              style: _emptyStyle,
            )
          ],
        ),
