...
class JoinController extends GetxController {
  ...
  void checkDuplicateId() async {
    await useCase.checkDuplicateId(idValue.value, successFunction: () {
      ...
      GAUtil.trackEvent(
          name: GAEventList.ID_INPUT_SUBMIT, params: {GAParameter.SUCCESS: 'true'});
      Get.toNamed(Routes.signUpAccountPwd);
    }, failFunction: (error) {
      GAUtil.trackEvent(
          name: GAEventList.ID_INPUT_SUBMIT,
          params: {GAParameter.SUCCESS: 'false'});
      ErrorHandler.manageError(error.errorCode, Get.currentRoute);
    });
  }
  ...
  RxInt certPhoneAttempCount = 0.obs;
  RxBool loading = false.obs;

  void certPhone(String text, Function() successFunction,
      {bool isResend = false}) async {
    if (isResend) {
      certPhoneAttempCount.value += 1;
    }
  ...
  void signUp() async {
    if (!signUpLoading.value) {
      ...
      await useCase.signUp(
        ...
        GAUtil.trackEvent(name: GAEventList.REVIEW_SUBMIT, params: {
