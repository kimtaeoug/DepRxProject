...
class LoginController extends GetxController {
  ...
  void onInit() {
    super.onInit();
    _socialLoginUtil.init(isSham: true);
  }
  ...
  void socialLogin({bool doForce = false}) async {
    await useCase.socialLogin(
      ...
      ErrorHandler.manageError(error.errorCode, Get.currentRoute,
          errorFunction: (code) {
        loginErrorProcess(code, isSocialLogin: true);
      });
    });
  }
  ...
  void socialLoginFunction(SocialType type) {
    switch (type) {
      case SocialType.google:
        GAUtil.trackEvent(
            name: GAEventList.GOOGLE_LOGIN_CLICK,
            params: {GAParameter.LOGIN_PROVIDER: 'google'}, isDeprx: false);
        loginWithGoogle();
        break;
      case SocialType.kakao:
        GAUtil.trackEvent(
            name: GAEventList.GOOGLE_LOGIN_CLICK,
            params: {GAParameter.LOGIN_PROVIDER: 'kakao'}, isDeprx: false);
        loginWithKakao();
        break;
      case SocialType.naver:
        GAUtil.trackEvent(
            name: GAEventList.GOOGLE_LOGIN_CLICK,
            params: {GAParameter.LOGIN_PROVIDER: 'naver'}, isDeprx: false);
        loginWithNaver();
        break;
    }
  }

  void checkNaverExisting(Function() successFunction)async{
    bool check = await _socialLoginUtil.checkNaverExisting();
    if(check){
      successFunction();
    }else{
      ToastHandler().showExceptionError(text: 'errorMsg_login'.tr);
